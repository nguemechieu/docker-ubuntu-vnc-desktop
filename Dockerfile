...
RUN apt update \
    ...
    && apt install -y --no-install-recommends supervisor nginx sudo net-tools zenity xz-utils \
...
# Desktop Environment
RUN apt update \
    && apt install -y --no-install-recommends lxde gtk2-engines-murrine gnome-themes-standard \
...
RUN apt update && apt install -y --no-install-recommends ffmpeg \
    && rm -rf /var/lib/apt/lists/* && mkdir /usr/local/ffmpeg && ln -s /usr/bin/ffmpeg /usr/local/ffmpeg/ffmpeg

# Python Libraries and Requirements
COPY rootfs/usr/local/lib/web/backend/requirements.txt /tmp/
RUN apt-get update && dpkg-query -W -f='${Package}\n' > /tmp/a.txt \
    ...
    && rm -rf /var/lib/apt/lists/* /var/cache/apt/* /tmp/a.txt /tmp/b.txt
...
RUN apt-get update && apt-get install -y --no-install-recommends curl ca-certificates gnupg patch
...
# Build frontend
COPY web /src/web
RUN cd /src/web && yarn && yarn build
...
# Expose ports and set health check
EXPOSE 80
WORKDIR /root
